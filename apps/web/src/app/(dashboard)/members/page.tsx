          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {isLoading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-8 text-center text-gray-400">No members found</td></tr>
              ) : (
                data?.members?.map((m: Record<string, unknown>) => {
                  ...
                  return (
                    <tr key={m.id as string} className="hover:bg-gray-50">
                      <td className="px-6 py-3 text-sm font-mono text-gray-600">{m.memberCode as string}</td>
                      <td className="px-6 py-3">
                        <Link href={`/members/${m.id}`} className="text-sm font-medium text-primary hover:underline">
                          {(m.user as Record<string, string>)?.name}
                        </Link>
                      </td>
                      <td className="px-6 py-3 text-sm text-gray-600">{(m.user as Record<string, string>)?.phone}</td>
